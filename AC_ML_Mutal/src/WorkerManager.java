	...
	public void handleGasWorkers()
	{
		...
		for (Unit unit : MyBotModule.Broodwar.self().getUnits())
		{
			...
			if (unit.getType().isRefinery() && unit.isCompleted() )
			{
				...
				// 저글링 이동속도 업그레이드를 하지 않은 경우 refinery에 드론 3명 보내기
				if(!MyBotModule.Broodwar.self().isUpgrading(UpgradeType.Metabolic_Boost) && MyBotModule.Broodwar.self().getUpgradeLevel(UpgradeType.Metabolic_Boost) == 0)
				{
				...
				// 저글링 이동속도 업그레이드 중이거나 완료했을 경우 드론 1마리만 남기고 미네랄 채취 명령
				if((MyBotModule.Broodwar.self().isUpgrading(UpgradeType.Metabolic_Boost) && MyBotModule.Broodwar.self().getUpgradeLevel(UpgradeType.Metabolic_Boost) == 0) ||
				   (!MyBotModule.Broodwar.self().isUpgrading(UpgradeType.Metabolic_Boost) && MyBotModule.Broodwar.self().getUpgradeLevel(UpgradeType.Metabolic_Boost) == 1)){
					if (getNumGasWorkers()>1){
						for (Unit gasUnit : workerData.getWorkers())
						{
							if (gasUnit == null) continue;
							if (gasUnit.isCompleted() && workerData.getWorkerJob(gasUnit) == WorkerData.WorkerJob.Gas)
							{
								workerData.setWorkerJob(gasUnit, WorkerData.WorkerJob.Minerals, unit);
							}
						}
					}
				}
