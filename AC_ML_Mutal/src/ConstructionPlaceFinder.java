...
public class ConstructionPlaceFinder {
    ...
    public final TilePosition getBuildLocationNear(UnitType buildingType, TilePosition desiredPosition) {
        ...
        if (desiredPosition == TilePosition.None || desiredPosition == TilePosition.Unknown || desiredPosition == TilePosition.Invalid || !desiredPosition.isValid()) {
                ...
                for (int i = bx0; i < bx4; i++) {
                    for (int j = by0; j < by3; j++) {
                        for (int k = mx0; k < mx2; k++) {
                            List<TilePosition> tileList = BWTA.getShortestPath(new TilePosition(i, j), new TilePosition(k, my0));
                            for (TilePosition t : tileList) {
                                tilesToAvoid.add(t);
                            }
                        }
